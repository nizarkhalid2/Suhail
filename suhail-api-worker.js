...
async function flights(p){
  ...
  const data=await duffel('/air/offer_requests?return_offers=true&view=offers','POST',body);
  return {provider:'duffel',offers:data?.data?.offers||[],request:data?.data?.id||null,liveMode:!!data?.data?.live_mode};
}
async function stays(p){
  ...
  const data=await duffel('/stays/search','POST',body);
...
export default {async fetch(req){
  ...
  try{
    if(u.pathname==='/api/health') return json({ok:true,provider:'duffel',configured:!!DUFFEL_TOKEN});
    if(req.method!=='POST') return json({error:'Method not allowed'},405);
    const p=await req.json();
    if(u.pathname==='/api/flights/search') return json(await flights(p));
    if(u.pathname==='/api/stays/search') return json(await stays(p));
